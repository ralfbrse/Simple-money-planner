...
class Menu:
    ...
    def user_select():
        ...
        while sel < 0:
            ...
            # Choose pre existing user
            if sel > 0 and sel <= len(data["Users"]):
                user = User()  # Instansiate blank user object
                # overright user with saved data
                user.load_user(data["Users"][user_list[sel]])
    ...
    def main_menu():
        print()
        options = ['edit hours', 'edit pay', 'calculate this month',
                   'view user', 'switch user', 'delete user', 'clear screen', 'quit']
